...
import { API_URL, Role, themeBg } from "../../../utils/variables";
...
const  CreateEpisode: NextPage = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        axios.post(`${API_URL}/episode/create`, formData, {withCredentials: true}).then((data) => {
            if(data.status === 201) {
                setEpisodeName('')
                setFile(undefined)
            }
    ...
    return (
        <>
            <Head>
                <title>Curs.RO | Create episode</title>
            </Head>
            <div className='px-8 md:px-24 py-3 items-center justify-center w-full flex flex-col'>
                <motion.div 
                    ...
                    className='w-full p-5 shadow-md rounded-md py-8 my-10 bg-white flex flex-col'>
                        <h2 className='font-semibold text-lg'>Create a new episode</h2>
                        <form onSubmit={handleSubmit} className='flex flex-col gap-2 mt-5 mx-auto justify-center w-full items-center md:justify-start md:items-start'>
                            ...
                            <button type='submit' className='p-2 bg-blue-500 rounded-sm text-white duration-200 hover:bg-black ml-auto mr-4'>Create episode</button>
                        </form>
                </motion.div>
            </div>
        </>
